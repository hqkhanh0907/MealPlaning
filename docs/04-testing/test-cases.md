...
> **v10.0**: Navigation Audit — Fix stale TC_NAV_03/04 (Grocery/Settings are NOT main tabs). Update TC_SHOP, TC_SET, TC_BAK preconditions. Add TC_ENERGY (5), TC_HEALTH (5). Settings accessed via header icon → full-screen page. Grocery accessed via Calendar > Bữa ăn button → modal.
>
...
- [Danh sách Test Cases (Browser QA Manual)](#danh-sách-test-cases-browser-qa-manual)
  ...
  - [TC_SHOP — Grocery List (Modal)](#tc_shop--grocery-list-modal)
  - [TC_SET — Settings (Full-screen Page)](#tc_set--settings-full-screen-page)
  ...
  - [TC_ENERGY — Energy Balance & Detail Sheet](#tc_energy--energy-balance--detail-sheet)
  - [TC_HEALTH — Health Profile Management](#tc_health--health-profile-management)
...
| --------- | ------------------------ | -------------------- | --------------------------------------------------- | ----------------------------------------------------------------- | ---------- |
| TC_NAV_01 | Chuyển tab Calendar      | App đang mở          | Click icon lịch (bottom nav)                        | Tab Calendar hiển thị với 2 sub-tabs: Bữa ăn / Dinh dưỡng         | ✅ Pass    |
| TC_NAV_02 | Chuyển tab Library       | App đang mở          | Click icon thư viện (bottom nav)                    | Tab Library hiển thị với 2 sub-tabs: Món ăn / Nguyên liệu         | ✅ Pass    |
| TC_NAV_03 | Chuyển tab AI Analysis   | App đang mở          | Click icon AI (bottom nav)                          | Tab AI Analysis hiển thị khu vực phân tích ảnh                    | ✅ Pass    |
| TC_NAV_04 | Chuyển tab Fitness       | App đang mở          | Click icon tạ (bottom nav)                          | Tab Fitness hiển thị với 3 sub-tabs: Kế hoạch / Tiến độ / Lịch sử | ✅ Pass    |
| TC_NAV_05 | Chuyển tab Dashboard     | App đang mở          | Click icon dashboard (bottom nav)                   | Tab Dashboard hiển thị tổng quan dinh dưỡng, tập luyện            | ✅ Pass    |
| TC_NAV_06 | Mở Settings (header)     | App đang mở          | Click icon Settings (SlidersHorizontal) trên header | Settings page full-screen overlay mở ra                           | ✅ Pass    |
| TC_NAV_07 | Mở Grocery (từ Calendar) | Calendar > Bữa ăn    | Click nút "Danh sách mua" trên Meals sub-tab        | GroceryModal overlay hiển thị                                     | ✅ Pass    |
...
### TC_SHOP — Grocery List (Modal)

| ID         | Tên                     | Điều kiện tiên quyết    | Bước thực hiện                                 | Kết quả mong đợi                                   | Trạng thái |
| ---------- | ----------------------- | ----------------------- | ---------------------------------------------- | -------------------------------------------------- | ---------- |
| TC_SHOP_01 | Tạo grocery từ kế hoạch | Tuần hiện tại có ≥1 bữa | Calendar > Bữa ăn > "Danh sách mua" → modal mở | Danh sách nguyên liệu cần mua với tổng số lượng    | ✅ Pass    |
...
### TC_SET — Settings (Full-screen Page)

| ID        | Tên                    | Điều kiện tiên quyết                                              | Bước thực hiện           | Kết quả mong đợi                           | Trạng thái |
| --------- | ---------------------- | ----------------------------------------------------------------- | ------------------------ | ------------------------------------------ | ---------- |
| TC_SET_01 | Cập nhật cân nặng      | Header icon Settings → Settings page > Hồ sơ sức khỏe > Chỉnh sửa | Sửa cân nặng → lưu       | Mục tiêu protein cập nhật (weight × ratio) | ✅ Pass    |
| TC_SET_02 | ~~Đổi ngôn ngữ~~       | —                                                                 | —                        | _Đã xóa — chỉ hỗ trợ Tiếng Việt (v7.0)_    | N/A        |
| TC_SET_04 | Cập nhật mục tiêu calo | Header icon Settings → Settings page > Mục tiêu > Chỉnh sửa       | Sửa targetCalories → lưu | Dashboard calo/ngày hiển thị target mới    | ✅ Pass    |
...
| ID        | Tên                      | Điều kiện tiên quyết                     | Bước thực hiện                                                              | Kết quả mong đợi                                 | Trạng thái |
| --------- | ------------------------ | ---------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------ | ---------- |
| TC_BAK_01 | Export dữ liệu           | Có dữ liệu trong app                     | Header icon Settings → Settings page > Quản lý dữ liệu > Export             | Hiển thị Android Share sheet, file JSON được tạo | ✅ Pass    |
| TC_BAK_02 | Import dữ liệu hợp lệ    | Có file backup JSON                      | Header icon Settings → Settings page > Quản lý dữ liệu > Import → chọn file | Dữ liệu được restore, toast thành công           | ✅ Pass    |
| TC_BAK_03 | Import file không hợp lệ | Settings page > Quản lý dữ liệu > Import | Chọn file JSON ngẫu nhiên không đúng schema                                 | Toast lỗi, dữ liệu hiện tại không bị ghi đè      | ✅ Pass    |
...
## TC_ENERGY — Energy Balance & Detail Sheet

| ID           | Tên                           | Điều kiện tiên quyết             | Bước thực hiện                   | Kết quả mong đợi                                                               | Trạng thái |
| ------------ | ----------------------------- | -------------------------------- | -------------------------------- | ------------------------------------------------------------------------------ | ---------- |
| TC_ENERGY_01 | Mở EnergyDetailSheet          | Dashboard tab, có health profile | Click energy-balance-mini widget | Bottom sheet mở ra với BMR, TDEE, Target, macros                               | 🔲         |
| TC_ENERGY_02 | Verify BMR/TDEE/Target        | TC_ENERGY_01                     | Xem các giá trị                  | BMR đúng Mifflin-St Jeor, TDEE = BMR × multiplier, Target = TDEE + goal offset | 🔲         |
| TC_ENERGY_03 | Per-meal breakdown            | TC_ENERGY_01                     | Xem per-meal section             | Tổng per-meal = Total eaten. Sáng/Trưa/Tối đúng                                | 🔲         |
| TC_ENERGY_04 | EnergyBalanceCard (Calendar)  | Calendar > Dinh dưỡng            | Xem EnergyBalanceCard            | Hiển thị eaten/target/remaining đúng, progress ring                            | 🔲         |
| TC_ENERGY_05 | Net calories = eaten - burned | Có meal plan + workout           | Check net calculation            | Net = eaten - workout calories burned. Hiển thị đúng                           | 🔲         |

---

## TC_HEALTH — Health Profile Management

| ID           | Tên                           | Điều kiện tiên quyết | Bước thực hiện                                           | Kết quả mong đợi                                                           | Trạng thái |
| ------------ | ----------------------------- | -------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------- | ---------- |
| TC_HEALTH_01 | Mở Health Profile             | App có profile       | Header Settings icon → Settings page → "Hồ sơ sức khỏe"  | Health Profile detail view hiển thị với thông tin hiện tại                 | 🔲         |
| TC_HEALTH_02 | Edit weight → recalculate     | TC_HEALTH_01         | "Chỉnh sửa" → đổi weight 75→80 → "Lưu"                   | BMR, TDEE, Target, macros tất cả recalculate. Dashboard, Calendar cập nhật | 🔲         |
| TC_HEALTH_03 | Change activity level         | TC_HEALTH_01         | "Chỉnh sửa" → đổi moderate→active → "Lưu"                | TDEE multiplier thay đổi (1.55→1.725). Target cập nhật                     | 🔲         |
| TC_HEALTH_04 | BMR Override mode             | TC_HEALTH_01         | "Chỉnh sửa" → toggle "Nhập thủ công" → nhập 1800 → "Lưu" | BMR=1800 (override). TDEE = 1800 × multiplier                              | 🔲         |
| TC_HEALTH_05 | Body Fat % → protein from LBM | TC_HEALTH_01         | "Chỉnh sửa" → nhập bodyFat=15% → "Lưu"                   | Protein tính từ LBM: (75×0.85)×ratio thay vì 75×ratio                      | 🔲         |
...
| 10.0    | 2026-07-05 | Navigation Audit: Fixed TC_NAV_03/04 (Grocery/Settings are NOT main tabs — they are modal/page accessed via other means). Updated TC_SHOP, TC_SET, TC_BAK preconditions. Added TC_ENERGY (5 TCs for EnergyDetailSheet/EnergyBalanceCard) and TC_HEALTH (5 TCs for Health Profile management). |
