...
1. **Tách [AIImageAnalyzer.tsx](src/components/AIImageAnalyzer.tsx)** → `ImageCapture` + `AnalysisResultView` + `SaveAnalyzedDishModal` + orchestrator. _(SRP+Composition — 45 phút)_
2. **Extract `toTempIngredient()` helper** → [nutrition.ts](src/utils/nutrition.ts), xóa duplicate. _(DRY — 5 phút)_
3. **Thêm type guards** cho migration functions trong [dataService.ts](src/services/dataService.ts). _(Type Safety — 15 phút)_
4. **Dọn "what" comments** → xóa hoặc đổi thành "why". _(Boy Scout — 10 phút)_
5. **Tạo [logger.ts](src/utils/logger.ts)** utility, thay 6x `console.error`. _(Observability — 10 phút)_
