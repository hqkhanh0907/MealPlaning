...
1. **Extract shared `ModalBackdrop`** → `shared/ModalBackdrop.tsx` + a11y attrs. Thay 4 copies + ~6 inline patterns. _(#2, #3 — 40 phút)_
2. **Xóa 3 local `MEAL_TYPE_LABELS`** + chuyển `MEAL_TYPE_ICONS` → `constants.ts`. _(#1 — 5 phút)_
3. **Extract `DishEditModal.tsx`** từ `DishManager`. _(#4 — 20 phút)_
4. **Refactor duplicate validation** trong `DishManager`/`IngredientManager`. _(#5 — 10 phút)_
5. **Extract `validateAndParseImportData()`** → `dataService.ts`. _(#6 — 15 phút)_
6. **Pre-compute nutrition** trong `DishManager.sortFn`. _(#10 — 10 phút)_
