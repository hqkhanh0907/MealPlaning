...
Last updated: 2026-05-16 (v1.12 — polish pass 12: cold-start performance investigation. Measured 3 runs on emulator-5554: TotalTime 5.0/5.2/5.5s avg. Visual phases via 22-frame screencap: 0-3s native splash logo, 4s blank cream (web init), 5s full Dashboard rendered. Capacitor splash hides via `SplashScreen.hide()` in `app.ts:35` afterNextRender. Earlier "16-20s splash" complaints (origin: pre-Pass 1 builds with sql.js full hydration on main thread) are STALE — current build is healthy. NO-OP pass; do not optimize cold-start unless re-measured on a real low-end device. Diagnostic frames: `/tmp/qa-audit/v7/cold-Ns.png`.)
