...
Last updated: 2026-05-16 (v1.15 — polish pass 15: Fitness tinh giản. **Audit before**: page 455 dòng HTML, 1 card "Chọn giáo án" gộp 3 widget (active-pill chip lặp tên plan + AI callout to 20% screen + plan-grid + week-strip), card "Buổi tập hôm nay" luôn render `.free-mode` picker với 100+ exercise chip + search input kể cả khi user không yêu cầu → noise dày đặc. **Root-cause fixes**: (1) Bỏ `active-pill` chip lặp tên plan trong section header `<p class="eyebrow">Giáo án</p>` (badge "Đang dùng" trên plan-card đã đủ); (2) AI callout `.ai-plan-callout--compact` modifier — `align-items: center`, `padding-y: xs`, drop paragraph, button `size="small"`, label "Tạo" thay "Tạo giáo án AI" → từ 20% screen còn ~6%; (3) Tách week-strip ra section riêng "LỊCH TUẦN" — chỉ render khi có `activePlan()`, không nhồi vào Giáo án card; (4) Thêm `freeModeOpen` signal + toggle button "Thêm bài tập tự do" (dashed border, 40px height) — free-mode picker `@if (freeModeOpen())` collapsed default; (5) `@if (!fitness.todayDay()?.is_rest_day)` gate cả toggle — rest day đã có CTA "Bắt đầu buổi tự do" không cần toggle trùng intent; (6) Section header trim: eyebrow "Buổi tập" → "Hôm nay", h2 "Tiến trình tập luyện" → "Khối lượng & sức mạnh"; (7) Chart-block empty branch `@if (length === 0)` đổi thành render conditional `@if (length > 0)` — bỏ "Chưa có buổi tập hoàn thành trong tuần này" placeholder paragraph (3 stat cards đã cover empty); (8) "Chuỗi tuần tập" → "Streak tuần tập", "Tính theo tuần, không phạt ngày nghỉ" → "Không phạt ngày nghỉ.". **Result**: above-the-fold trước = Giáo án header + AI callout to + 1 plan card peek; sau = Giáo án header + AI callout compact + 2 plan cards full visible. Page density ~30% giảm noise. 802/802 tests, 9/9 guards. Screenshots: `/tmp/qa-audit/v10/20–22`. **K.20 added**. **Deferred**: weight card (60 kg) hiện trong "Khối lượng & sức mạnh" → nên tách section "Cơ thể" (mismatch metric); empty Tiến trình thêm CTA "Ghi buổi tập đầu tiên".)
