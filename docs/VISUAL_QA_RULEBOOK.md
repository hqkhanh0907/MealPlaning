...
Last updated: 2026-05-16 (v1.9 — polish pass 9: K.18 NEW canonical edit-page chrome — Save → `<ion-footer>` `.sticky-save-bar`, Delete → toolbar `slot="end"` trash icon w/ existing confirm-dialog, tab bar hidden via `body.edit-overlay-open` body-class hook. Refactor applied to `dish-edit.page` + `ingredient-edit.page`. Now matches Settings sub-pages (`goals-edit`/`body-edit`/`activity-edit`) that already used `.sticky-save-bar`. Drops per-page `.btn-cta` `:host` override and `padding-bottom: 220px` hacks. Verified on emulator-5554: trash icon top-right, tab bar hidden, sticky LƯU THAY ĐỔI pinned bottom w/ safe-area inset. 802/802 unit tests pass. 5 CI guards pass. Screenshots: `/tmp/qa-audit/v4/`.)
