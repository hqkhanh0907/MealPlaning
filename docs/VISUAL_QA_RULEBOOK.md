...
Last updated: 2026-05-16 (v1.13 — polish pass 13: Long-VN-name + edge data audit. (1) Existing seed names max 21 chars (dish) / 16 chars (ingredient) — no organic overflow witnessed across Dishes/Ingredients lists, macros row clean. (2) Confirmed FAB occluded last card kebab in Management lists — `.management-shell` `padding-bottom` was `96px` (exact FAB bottom), so last card's bottom edge sat at FAB's TOP edge → kebab unreachable. Root-cause fix: `padding-bottom: calc(168px + env(safe-area-inset-bottom, 0px))` = FAB bottom (96) + FAB height (56) + gap (16). Verified scroll-to-end on emulator-5554 — FAB now floats above the last card with breathing room. (3) Filter chips have soft-mask scroll fade already (theme/list-card.scss:31-44) — vision falsely flagged "clipped"; no fix needed. Long-name DB seeding stress test deferred (invasive seed change for marginal gain — current names won't grow >25 chars per PRD). 802/802 tests, design-tokens guard pass. Screenshots: `/tmp/qa-audit/v8/`.)
