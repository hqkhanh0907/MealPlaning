...
Last updated: 2026-05-16 (v1.14 — polish pass 14: Dashboard tinh giản, ngôn ngữ Apple. **Audit before**: Dashboard có 8 section + 6 quick-action, empty-state lặp 3 lần (Hero "Hôm nay chưa có món" + AI Insight "Chưa có dữ liệu" + Nutrition all-zero), refresh button toolbar = decision noise, quick actions duplicate tab bar ("Quản lý món ăn", "Tập luyện", "Cập nhật cơ thể"), Hero card chiếm ~40% screen vì `font-size-display` + `space-xl` padding + copy dài 3 dòng. **Root-cause fixes**: (1) `DashboardStore.insight()` empty-state branch trả `null` (line 197) — Hero card đã cover thông điệp này, không cần card thứ 2; (2) Bỏ refresh button + `refresh()` method khỏi `dashboard.page.ts` (auto-refresh `ionViewWillEnter` đã đủ); (3) Quick actions 6→2, chỉ giữ "Tạo AI insight" + "Phân tích ảnh món ăn" — đây mới là entry-point unique không có ở tab bar; (4) Hero SCSS: `font-size-display` → `font-size-headline`, `padding xl` → `lg`, line-height 1.2 → 1.25, body font `body` → `body-sm`; (5) `heroTitle` copy rút gọn: "Hôm nay chưa có món nào trong lịch ăn" → "Sẵn sàng cho hôm nay", "Kế hoạch hôm nay vượt mục tiêu" → "Vượt mục tiêu kcal"; `heroBody` chỉ render 1 dòng ngắn; (6) Macro-list trong nutrition card gated bằng `@if (hasNutritionData())` — khi all-zero ẩn 4 dòng macro/progress (Protein 0%, Carbs Chưa có mục tiêu, Fat Chưa có mục tiêu, Fiber 0%) chỉ giữ ring + calorie summary; (7) Workout card riêng bị merge: section "Tiến độ" gộp streak + workout-streak + weight thành 1 grid 3 card, section header rút "Chuỗi ngày, cân nặng & tập luyện" → "Streak & cân nặng"; (8) AI Insight + Photo Analysis cards giữ nguyên nhưng chỉ render khi có data thật (`@if (dailyAiInsight())`, `@if (foodImageAnalysis())`). **Result before/after**: 8 sections → 5 sections + 2 conditional, Hero ~620px → ~340px (~40% → ~22% screen), above-the-fold giờ chứa Hero + Nutrition + section-header Tiến độ + 2 stat tiles thay vì chỉ Hero. 802/802 tests, 9/9 guards. Screenshots: `/tmp/qa-audit/v9/95–98`. **K.19 added** below. **Deferred (next pass)**: Nutrition ring all-zero → illustration/CTA "Ghi bữa đầu tiên" thay ring rỗng, asymmetric "ngày"/"tuần" label trong streak cards, "Dữ liệu từ hồ sơ cá nhân" subtitle trên weight card → CTA "Cập nhật", typography hierarchy giữa Hero h1 và nutrition kcal number.)
