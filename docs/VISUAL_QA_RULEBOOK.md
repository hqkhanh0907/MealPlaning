...
Last updated: 2026-05-16 (v1.8 — polish pass 8: K.12 extended to week-view range copy `Thg N → tháng N`; K.13 EN-leak sweep applied to Fitness page — `Tạo AI plan → Tạo giáo án AI`, `Free workout → buổi tự do`, `Guided/Free pill → Theo giáo án/Tự do`, `guided mode → theo giáo án`, `Weight/Reps/Rest → Tạ/Số lần/Nghỉ`, `Log set → Ghi set`, `Giáo án AI tùy chỉnh → Giáo án AI riêng cho bạn`. No new K patterns — these were already codified and re-applied where regressions accumulated. Audit captured 12 baseline screenshots in `/tmp/qa-audit/` and 4 verification shots in `/tmp/qa-audit/v2/`. UI consistency matrix §J.1 unchanged — verified all rows still hold on emulator.)
