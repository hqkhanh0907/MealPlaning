...
Last updated: 2026-05-16 (v1.10 — polish pass 10: Logging Modal eyebrow contrast fix. `.logging-modal__eyebrow` was `--text-tertiary` (designed for cream BG) but rendered on sage-green header BG → effectively invisible. Switched to `var(--ion-color-primary-contrast)` (white) + `opacity: 0.78` + uppercase `letter-spacing: 0.08em` for eyebrow signature. Verified on emulator-5554: eyebrow now legibly white on sage. 802/802 tests pass. Design-token guard pass (one new `// allow-hardcode` for opacity tier). Pass 10 deferred (architectural, not visual): L2 global-stepper-in-header semantic ambiguity (needs PRD validation), L4 thumbnail per item (no image storage in PRD). Screenshots: `/tmp/qa-audit/v5/`.)
