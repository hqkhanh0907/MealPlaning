...
7. System persist vào SQLite (bảng `day_plans`) qua `dayPlanStore`
...
7. System lưu vào SQLite (bảng `ingredients`) qua `ingredientStore`
...
4. System persist tất cả vào SQLite qua các Zustand stores (optimistic update)
...
3. System persist `healthProfile` vào SQLite (bảng `health_profiles`) qua `healthProfileStore`
...
4. System persist ngôn ngữ vào `localStorage` (via i18next — Vietnamese only trong V1)
...
7. System persist kế hoạch mới vào SQLite (bảng `day_plans`) qua `dayPlanStore`
...
5. System persist vào SQLite (bảng `meal_templates`) qua `mealTemplateStore`
...
## UC-19: User Onboarding with Plan Strategy Choice
...
## UC-20: View Weekly Training Plan
...
## UC-21: Edit Exercises on a Training Day
...
## UC-22: Add/Manage Multiple Workout Sessions
...
## UC-23: Toggle Day Between Workout/Rest
...
## UC-24: Log a Workout with Weight/Reps
...
## UC-27: View Daily Dashboard Summary
...
## UC-28: Use AI to Analyze Food Image
