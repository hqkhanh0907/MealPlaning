...
> **v3.0 — Cloud Sync keys**: Thêm `mp-auth-state`, `mp-last-sync-at`, `meal-templates` keys cho Google Drive sync và meal template features.
...
| `mp-auth-state` | `{ user: { name: string; email: string; picture: string } \| null; accessToken: string \| null }` | Google OAuth2 authentication state | — |
| `mp-last-sync-at` | `string \| null` (ISO 8601) | Timestamp of last successful Google Drive sync | — |
| `meal-templates` | `MealTemplate[]` | User-saved meal plan templates | — |
...
### `mp-auth-state` — Google OAuth2 State

```typescript
// src/types.ts
type AuthState = {
  user: {
    name: string;      // Google account display name
...
**Lưu ý:**
- Persisted to survive page refreshes
- Token có thể hết hạn — app tự refresh khi cần
- Khi user logout, reset về default value

---

### `mp-last-sync-at` — Last Sync Timestamp

```typescript
// ISO 8601 timestamp hoặc null
type LastSyncAt = string | null;
```
...
**Lưu ý:**
- Dùng cho conflict detection khi sync với Google Drive
- Giá trị `null` nghĩa là chưa từng sync
- Cập nhật sau mỗi lần sync thành công

---

### `meal-templates` — `MealTemplate[]`

```typescript
// src/types.ts
type MealTemplate = {
  id: string;              // generateId('tmpl') → "tmpl-{timestamp}-{random}"
  name: LocalizedString;   // { vi: "Meal prep tuần", en: "Weekly Meal Prep" }
  dishIds: string[];       // FK → Dish.id — danh sách món trong template
...
[
  {
    "id": "tmpl-1710000000000-a1b2c3d",
    "name": { "vi": "Meal prep tuần", "en": "Weekly Meal Prep" },
    "dishIds": ["dish-1709123456790-x7y8z", "dish-1709123456793-p4q5r"],
...
**Lưu ý:**
- Mỗi template lưu danh sách `dishIds` có thể áp dụng cho bất kỳ ngày nào
- Khi áp dụng template, app copy dish IDs vào `DayPlan` tương ứng
- Orphan references: nếu xóa Dish, template vẫn giữ ID nhưng dish bị bỏ qua khi render
...
| **Backup** | Export tạo file JSON gồm tất cả 7 keys, có thể Share (Android) hoặc download (web). |
...
const totalBytes = ['mp-ingredients', 'mp-dishes', 'mp-day-plans', 'mp-user-profile', 'mp-auth-state', 'mp-last-sync-at', 'meal-templates']
