...
#### ⚠️ Persistence Gap — Known Limitation (CEO Audit)

> **CRITICAL**: 4 stores chỉ update Zustand in-memory state khi mutation. Dữ liệu **CHỈ** được load từ SQLite lên khi khởi động (`loadAll()`), nhưng các thao tác thêm/sửa/xóa **KHÔNG** ghi ngược xuống SQLite.

| Store                | `loadAll()` (đọc SQLite → Zustand) | Mutation ghi SQLite                    | Cơ chế persist thực tế       |
| -------------------- | ---------------------------------- | -------------------------------------- | ---------------------------- |
| `dayPlanStore`       | ✅ Có                              | ❌ **Không**                           | Chỉ Google Drive sync (JSON) |
| `dishStore`          | ✅ Có                              | ❌ **Không**                           | Chỉ Google Drive sync (JSON) |
| `ingredientStore`    | ✅ Có                              | ❌ **Không**                           | Chỉ Google Drive sync (JSON) |
| `mealTemplateStore`  | ✅ Có                              | ❌ **Không**                           | Chỉ Google Drive sync (JSON) |
...
1. **Web (sql.js)**: Dữ liệu của 4 stores trên tồn tại trong RAM. Reload trang = mất data (trừ khi đã Google Drive sync).
2. **Android native**: `NativeDatabaseService` persist SQLite trên disk, nhưng vì store mutations không ghi DB, data mới chỉ tồn tại trong Zustand memory cho đến khi app restart.
3. **Google Drive sync** (`useAutoSync`) là cơ chế backup duy nhất cho 4 stores: hook subscribe các store → debounce 3s → `exportToJSON()` → upload Google Drive. Download sẽ `importFromJSON()` + `reloadAllStores()`.
4. **`fitnessStore`** là **store duy nhất** dùng `persistToDb()` từ `dbWriteQueue` cho individual mutations, kết hợp `db.transaction()` cho atomic operations.
...
### 12.1 Persistence Write-Back Gap

**4 stores** (`dayPlanStore`, `dishStore`, `ingredientStore`, `mealTemplateStore`) thiếu SQLite write-back cho mutations. Dữ liệu chỉ được load từ SQLite lên Zustand khi startup (`loadAll()`), nhưng các thao tác CRUD chỉ update Zustand in-memory state. Xem chi tiết tại [§2.4 Persistence Gap](#⚠️-persistence-gap--known-limitation-ceo-audit).

**Mitigation hiện tại**: `useAutoSync` hook subscribe 4 stores → debounce 3s → export toàn bộ DB sang JSON → upload Google Drive. Đây là cơ chế backup duy nhất.

**So sánh với fitnessStore**: `fitnessStore` implement đầy đủ `persistToDb()` + `db.transaction()` cho mọi mutation — là model chuẩn cho việc refactor 4 stores còn lại.
...
Database schema đã phát triển lên version 6 với 22+ tables (được tạo qua `CREATE TABLE IF NOT EXISTS` trong `schema.ts`). Ngoài ra có migration logic tạo thêm tables trong quá trình upgrade (ví dụ: `plan_templates`, `training_plan_days_v2`).
