...
export function evaluateAndSuggestAdjustment(
  ...
  const recentEntries = getEntriesInWindow(
    weightLog,
    0,
    AUTO_ADJUST_CONFIG.evaluationPeriodDays,
  );
  if (recentEntries.length < AUTO_ADJUST_CONFIG.minWeightEntries)
    return null;
  ...
  if (goalType === 'cut' && weightChange >= -weightChangeThreshold) {
    ...
    const reason =
      weightChange > 0
        ? 'Weight is increasing during cut phase'
  ...
  if (goalType === 'bulk' && weightChange <= weightChangeThreshold) {
    ...
    const reason =
      weightChange < 0
        ? 'Weight is decreasing during bulk phase'
...
export function calculateAdherence(
  ...
  const calorieDays = Math.min(
    actualCalories.length,
    targetCalories.length,
  );
  const proteinDays = Math.min(
    actualProtein.length,
    targetProtein.length,
  );

  if (calorieDays === 0 && proteinDays === 0)
  ...
  return {
    calorie:
      calorieDays > 0
        ? Math.round((calorieHitDays / calorieDays) * 100)
        : 0,
    protein:
      proteinDays > 0
        ? Math.round((proteinHitDays / proteinDays) * 100)
        : 0,
  };
}
...
export function useFeedbackLoop(): {
  ...
  const movingAverage = useMemo(
    () => calculateMovingAverage(weightEntries),
    [weightEntries],
  );

  const adjustment = useMemo(() => {
    if (dismissed || applied || !activeGoal) return null;
    return evaluateAndSuggestAdjustment(
      weightEntries,
      targetCalories,
      activeGoal.type,
      tdee,
  ...
  const adherence = useMemo<AdherenceResult>(
    () => ({ calorie: 0, protein: 0 }),
    [],
  );
