...
import type { DayNutritionSummary, MealType, SaveAnalyzedDishPayload } from './types';
...
const AIImageAnalyzer = React.lazy(() =>
  import('./components/AIImageAnalyzer').then(m => ({ default: m.AIImageAnalyzer })),
);
const ManagementTab = React.lazy(importManagementTab);
const FitnessTab = React.lazy(() =>
  import('./features/fitness/components/FitnessTab').then(m => ({ default: m.FitnessTab })),
);
const DashboardTab = React.lazy(() =>
  import('./features/dashboard/components/DashboardTab').then(m => ({ default: m.DashboardTab })),
);
...
const WorkoutLogger = React.lazy(() =>
  import('./features/fitness/components/WorkoutLogger').then(m => ({ default: m.WorkoutLogger })),
);
const CardioLogger = React.lazy(() =>
  import('./features/fitness/components/CardioLogger').then(m => ({ default: m.CardioLogger })),
);
const PlanDayEditor = React.lazy(() =>
  import('./features/fitness/components/PlanDayEditor').then(m => ({ default: m.PlanDayEditor })),
);
const PlanScheduleEditor = React.lazy(() => import('./features/fitness/components/PlanScheduleEditor'));
const SplitChanger = React.lazy(() =>
  import('./features/fitness/components/SplitChanger').then(m => ({ default: m.SplitChanger })),
);
...
const MealPlannerModal = React.lazy(() =>
  import('./components/modals/MealPlannerModal').then(m => ({ default: m.MealPlannerModal })),
);
...
const CopyPlanModal = React.lazy(() =>
  import('./components/modals/CopyPlanModal').then(m => ({ default: m.CopyPlanModal })),
);
const TemplateManager = React.lazy(() =>
  import('./components/modals/TemplateManager').then(m => ({ default: m.TemplateManager })),
);
...
const UnifiedOnboarding = React.lazy(() =>
  import('./components/UnifiedOnboarding').then(m => ({ default: m.UnifiedOnboarding })),
);
  ...
  const renderPage = (): React.ReactNode => {
    ...
    switch (page.component) {
      case 'WorkoutLogger':
        return (
          <WorkoutLogger
            planDay={props.planDay as WorkoutLoggerPlanDay}
      ...
      case 'SplitChanger':
        return (
          <SplitChanger
            ...
            currentSplit={props.currentSplit as SplitChangerSplitType}
            onComplete={onBack}
          />
        );
...
export default function App() {
  ...
  const handleEditAISuggestionMeal = useCallback(
    (type: MealType) => {
  ...
  const handleClearPlan = useCallback(
    (scope: 'day' | 'week' | 'month', meals?: MealType[]) => {
  ...
  const handleSaveAnalyzedDish = useCallback(
    (result: SaveAnalyzedDishPayload) => {
      ...
      if (result.shouldCreateDish === false) {
        notify.success(
          ...
          t('notification.savedIngredients', { count: newIngredients.length }),
        );
        navigateTab('library');
        ...
        notify.success(
          t('notification.saveSuccess'),
          t('notification.savedDish', { name: result.name, count: newIngredients.length }),
        );
        navigateTab('library');
  ...
  const handleCopyPlanAction = useCallback(
    (targetDates: string[], mergeMode: boolean) => {
      const snapshot = dayPlans
        .filter(p => targetDates.includes(p.date))
        .map(p => ({
          ...p,
          breakfastDishIds: [...p.breakfastDishIds],
          lunchDishIds: [...p.lunchDishIds],
          dinnerDishIds: [...p.dinnerDishIds],
        }));
  ...
  const handleAnalysisComplete = useCallback(() => {
    ...
    if (useNavigationStore.getState().activeTab !== 'ai-analysis') {
      ...
      notify.success(t('notification.analysisComplete'), t('notification.analysisCompleteHint'), {
        onClick: () => navigateTab('ai-analysis'),
      });
    }
      ...
      <main id="main-content" className="mx-auto max-w-5xl px-4 py-6 pb-28 sm:px-6 sm:py-8 sm:pb-8">
        ...
        >
          <ErrorBoundary fallbackTitle={t('errorBoundary.calendarTab')}>
            <CalendarTab
              ...
              onSaveTemplate={modals.openSaveTemplate}
        ...
        >
          <ErrorBoundary fallbackTitle={t('errorBoundary.managementTab')}>
            <Suspense fallback={<TabLoadingFallback />}>
              <ManagementTab
                activeSubTab={activeManagementSubTab}
                onSubTabChange={setActiveManagementSubTab}
                ingredients={ingredients}
                dishes={dishes}
                onAddIngredient={addIngredient}
                onUpdateIngredient={updateIngredient}
                onDeleteIngredient={handleDeleteIngredient}
                isIngredientUsed={isIngredientUsed}
                onAddDish={addDish}
                onUpdateDish={updateDish}
                onDeleteDish={deleteDish}
                isDishUsed={isDishUsed}
              />
            </Suspense>
          </ErrorBoundary>
        </div>
          ...
          <MealPlannerModal
            ...
            onConfirm={changes => {
              ...
              if (changedCount === 1) {
                ...
                notify.success(
                  t('notification.planUpdated'),
                  t('notification.planUpdatedDesc', { count: totalDishes, meal: mealLabel }),
                );
              } else {
                notify.success(
                  t('notification.planUpdated'),
                  t('notification.planUpdatedDesc', {
                    count: totalDishes,
                    meal: `${changedCount} ${t('common.item')}`,
                  }),
                );
              }
            }}
