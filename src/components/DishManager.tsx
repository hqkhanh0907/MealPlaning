    ...
    <div data-testid="dish-manager" className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide -mt-2">
          <button onClick={() => setFilterTag(null)} data-testid="btn-filter-all-dishes" className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all min-h-11 ${filterTag ? 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700' : 'bg-emerald-500 text-white'}`}>
          ...
          {mealTagOptions.map(({ type, label, icon: TagIcon }) => {
            ...
            return (
              <button key={type} onClick={() => setFilterTag(filterTag === type ? null : type)} data-testid={`btn-filter-${type}`} className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all min-h-11 inline-flex items-center gap-1 ${filterTag === type ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
          ...
          {list.filteredItems.map(dish => {
            ...
            return (
              <div key={dish.id} className={`relative bg-white dark:bg-slate-800 p-5 rounded-2xl border shadow-sm hover:shadow-md transition-all flex flex-col group text-left w-full ${compareIds.has(dish.id) ? 'border-blue-400 dark:border-blue-500 ring-2 ring-blue-100 dark:ring-blue-900/50' : 'border-slate-100 dark:border-slate-700'}`}>
                <button data-testid={`btn-compare-${dish.id}`} onClick={() => toggleCompare(dish.id)} className={`absolute top-3 right-3 z-10 w-7 h-7 min-h-11 min-w-11 rounded-lg flex items-center justify-center border-2 transition-all ${compareIds.has(dish.id) ? 'bg-blue-500 border-blue-500 text-white' : 'border-slate-200 dark:border-slate-600 text-transparent hover:border-blue-400 hover:text-blue-400'}`} aria-label={t('dish.compare')}>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  <div className="bg-emerald-50 dark:bg-emerald-900/30 rounded-lg p-2 flex items-center justify-between"><span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase">{t('common.calories')}</span><span className="text-sm font-bold text-emerald-700 dark:text-emerald-300">{Math.round(nutrition.calories)}</span></div>
                  ...
                  return (
                    <tr key={dish.id} className={`transition-colors ${compareIds.has(dish.id) ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                      ...
                      <td className="px-4 py-3"><div className="flex items-center justify-end gap-3">
                        <button data-testid={`btn-clone-dish-${dish.id}`} onClick={() => handleClone(dish)} aria-label={`${t('common.clone')} ${getLocalizedField(dish.name, lang)}`} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center"><Copy className="w-4 h-4" /></button>
                        <button data-testid={`btn-edit-dish-${dish.id}`} onClick={() => modal.openEdit(dish)} aria-label={`${t('common.edit')} ${getLocalizedField(dish.name, lang)}`} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center"><Edit3 className="w-4 h-4" /></button>
                        <button data-testid={`btn-delete-dish-${dish.id}`} onClick={() => handleDelete(dish.id, getLocalizedField(dish.name, lang))} aria-disabled={isUsed(dish.id)} aria-label={`${t('common.delete')} ${getLocalizedField(dish.name, lang)}`} className={`p-2 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center ${isUsed(dish.id) ? 'text-slate-300 dark:text-slate-500 opacity-40' : 'text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30'}`}><Trash2 className="w-4 h-4" /></button>
                      </div></td>
                    </tr>
                  );
                })}
              ...
              return (
                <div key={dish.id} className={`relative p-4 flex items-center justify-between gap-3 transition-colors w-full text-left ${compareIds.has(dish.id) ? 'bg-blue-50 dark:bg-blue-900/20' : 'active:bg-slate-50 dark:active:bg-slate-700'}`}>
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <button data-testid={`btn-compare-${dish.id}`} onClick={() => toggleCompare(dish.id)} className={`w-7 h-7 min-h-11 min-w-11 rounded-lg flex items-center justify-center border-2 shrink-0 relative z-10 transition-all ${compareIds.has(dish.id) ? 'bg-blue-500 border-blue-500 text-white' : 'border-slate-200 dark:border-slate-600 text-transparent hover:border-blue-400'}`} aria-label={t('dish.compare')}><GitCompareArrows className="w-3.5 h-3.5" /></button>
                  ...
                  <div className="relative z-10 flex items-center gap-3 shrink-0">
                    <button data-testid={`btn-clone-dish-${dish.id}`} onClick={() => handleClone(dish)} aria-label={`${t('common.clone')} ${getLocalizedField(dish.name, lang)}`} className="p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center"><Copy className="w-4 h-4" /></button>
                    <button data-testid={`btn-edit-dish-${dish.id}`} onClick={() => modal.openEdit(dish)} aria-label={`${t('common.edit')} ${getLocalizedField(dish.name, lang)}`} className="p-2.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center"><Edit3 className="w-4 h-4" /></button>
                    <button data-testid={`btn-delete-dish-${dish.id}`} onClick={() => handleDelete(dish.id, getLocalizedField(dish.name, lang))} aria-disabled={isUsed(dish.id)} aria-label={`${t('common.delete')} ${getLocalizedField(dish.name, lang)}`} className={`p-2.5 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center ${isUsed(dish.id) ? 'text-slate-300 dark:text-slate-500 opacity-40' : 'text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30'}`}><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              );
            })}
        ...
        return (
          <DetailModal title={t('dish.detail')} editLabel={t('dish.editDish')} onClose={modal.closeView} onEdit={() => modal.openEditFromView(dish)}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-emerald-50 dark:bg-emerald-900/30 rounded-xl p-3.5"><p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase mb-1">{t('common.calories')}</p><p className="text-xl font-bold text-emerald-700 dark:text-emerald-300">{Math.round(nutrition.calories)} <span className="text-xs font-medium text-emerald-600">kcal</span></p></div>
          ...
          <dialog data-testid="compare-panel" open className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center m-0 w-full h-full border-none p-0 bg-transparent" aria-label={t('dish.compareNutrition')}>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[80vh] overflow-y-auto shadow-2xl relative z-10">
              <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-800 z-10">
                ...
                <button onClick={() => setShowCompare(false)} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg transition-all min-h-11 min-w-11 flex items-center justify-center" aria-label={t('common.close')}><X className="w-5 h-5" /></button>
              </div>
              <div className="p-4">
                <table className="w-full">
                  <thead>
                    <tr>
                      <th className="text-left py-2 px-2 text-xs font-bold text-slate-500 uppercase">&nbsp;</th>
                      {compareDishes.map(d => (
                        ...
                        <tr key={key} className="border-t border-slate-50 dark:border-slate-700">
                          ...
                          {values.map((v, i) => (
                            <td key={compareDishes[i].id} className="text-center py-2.5 px-2">
                              ...
                              {key === 'calories' && <span className="text-xs text-slate-500 ml-0.5">kcal</span>}
                            </td>
                          ))}
                        </tr>
                      );
