      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl shadow-xl w-full sm:max-w-2xl h-[90vh] sm:h-auto sm:max-h-[90vh] overflow-hidden flex flex-col sm:mx-4">
        <div className="px-4 py-4 sm:px-8 sm:py-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={onBack} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-all min-h-11 min-w-11 flex items-center justify-center">
              <ChevronRight className="w-6 h-6 rotate-180" />
            </button>
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-slate-800">Chọn món cho {MEAL_TYPE_LABELS[planningType]}</h3>
              <p className="text-xs sm:text-sm text-slate-500">Chọn các món ăn cho bữa này</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-all min-h-11 min-w-11 flex items-center justify-center">
            <X className="w-6 h-6" />
          </button>
        </div>

        <div className="px-4 py-3 sm:px-8 border-b border-slate-100 bg-slate-50">
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 outline-none bg-white shadow-sm text-base"
              />
            </div>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as SortOption)}
              className="w-full sm:w-48 px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 outline-none bg-white shadow-sm text-slate-700 font-medium text-base sm:text-sm min-h-11"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-3">
          {filteredDishes.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 font-medium">Chưa có món ăn phù hợp cho {MEAL_TYPE_LABELS[planningType]}.</p>
              <p className="text-slate-400 text-sm mt-1">Hãy thêm món ăn và gắn tag &quot;{MEAL_TYPE_LABELS[planningType]}&quot; trong Thư viện.</p>
            </div>
          )}
          {filteredDishes.map(({ dish, nutrition }) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 sm:p-5 rounded-2xl border-2 transition-all flex items-center justify-between group min-h-16 active:scale-[0.98] ${isSelected ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-100 hover:border-emerald-200'}`}
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isSelected ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-50 text-slate-400'}`}>
                    <ChefHat className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <h4 className={`font-bold text-base truncate ${isSelected ? 'text-emerald-900' : 'text-slate-800'}`}>{dish.name}</h4>
                    <div className="flex gap-3 mt-0.5">
                      <span className="text-xs font-bold text-slate-500">{Math.round(nutrition.calories)} kcal</span>
                      <span className="text-xs font-bold text-blue-500">{Math.round(nutrition.protein)}g Pro</span>
                    </div>
                  </div>
                </div>
                <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all shrink-0 ml-3 ${isSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-200 text-transparent group-hover:border-emerald-300'}`}>
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-5 border-t border-slate-100 bg-white">
          <div className="flex items-center justify-between mb-3 text-sm">
            <span className="text-slate-500 font-medium">
              Đã chọn: <span className="font-bold text-slate-800">{selectedCount} món</span>
            </span>
            {selectedCount > 0 && (
