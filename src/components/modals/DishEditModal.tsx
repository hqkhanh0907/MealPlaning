...
export const DishEditModal: React.FC<DishEditModalProps> = ({
  ...
  const [qaUnit, setQaUnit] = useState('g');
  ...
  const triggerAIFill = useCallback((name: string, unit: string) => {
    ...
    aiTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (!ctrl.signal.aborted) setQaAiLoading(false);
      }
    }, 800);
  }, []);
  ...
  const handleQuickCreate = useCallback(() => {
    ...
    const newIng: Ingredient = {
      ...
      unit: { vi: qaUnit.trim() || 'g', en: qaUnit.trim() || 'g' },
    ...
    onCreateIngredient?.(newIng);
    handleAddIngredient(newIng.id);
    resetQuickAdd();
  }, [qaNameVI, qaNameEN, qaUnit, qaCal, qaProtein, qaCarbs, qaFat, qaFiber, t, onCreateIngredient, handleAddIngredient, resetQuickAdd]);
            ...
            <div className="space-y-3">
              ...
              {showQuickAdd && (
                <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 space-y-3">
                  <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">{t('dish.quickAddTitle')}</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        onBlur={() => triggerAIFill(qaNameVI, qaUnit)}
                        placeholder={t('dish.quickAddNameVIPlaceholder')}
                        className={`w-full px-3 py-2 text-sm rounded-lg border ${qaError ? 'border-rose-500' : 'border-slate-200 dark:border-slate-600'} bg-white dark:bg-slate-700 dark:text-slate-100 outline-none focus:border-emerald-500 transition-all`}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-slate-100 outline-none focus:border-emerald-500 transition-all"
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onBlur={() => triggerAIFill(qaNameVI, qaUnit)}
                      placeholder={t('dish.quickAddUnitPlaceholder')}
                      className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-slate-100 outline-none focus:border-emerald-500 transition-all"
                    />
                  </div>
                  <div>
                  ...
                  <div className="flex gap-2 pt-1">
                    <button type="button" onClick={resetQuickAdd} data-testid="btn-qa-cancel" className="flex-1 py-2 rounded-lg text-sm font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">
                      {t('dish.quickAddCancel')}
                    </button>
                    <button type="button" onClick={handleQuickCreate} data-testid="btn-qa-submit" className="flex-[2] py-2 rounded-lg text-sm font-bold text-white bg-emerald-500 hover:bg-emerald-600 transition-all flex items-center justify-center gap-1.5">
