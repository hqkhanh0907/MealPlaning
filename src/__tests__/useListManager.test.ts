...
const searchFn = (item: TestItem, query: string) => item.name.toLowerCase().includes(query.toLowerCase());
...
describe('useListManager', () => {
  it('returns all items sorted by default', () => {
    const { result } = renderHook(() => useListManager({ items, searchFn, sortFn, defaultSort: 'name' as const }));
