...
describe('calculateDishNutrition', () => {
  ...
  it('should return zero for empty ingredients', () => {
    const dish: Dish = { id: 'dish-2', name: { vi: 'Empty', en: 'Empty' }, ingredients: [], tags: ['lunch'] };
    const result = calculateDishNutrition(dish, allIngredients);
    expect(result.calories).toBe(0);
    expect(result.protein).toBe(0);
    expect(result.carbs).toBe(0);
...
describe('toTempIngredient', () => {
  it('should map AnalyzedIngredient to Ingredient with empty id', () => {
    ...
    expect(result.name).toEqual({ vi: 'Ức gà', en: 'Ức gà' });
  });
  ...
  it('should apply normalizeUnit to the unit field', () => {
    ...
    const result = toTempIngredient(analyzed);
    expect(result.unit).toEqual({ vi: 'g', en: 'g' }); // "Grams" → "g"
  });
  ...
  it('should preserve non-standard units as-is (lowercased)', () => {
    ...
    const result = toTempIngredient(analyzed);
    expect(result.unit).toEqual({ vi: 'quả', en: 'quả' }); // Unknown unit → lowercased
  });
