...
describe('SubTabBar', () => {
  it('renders all tabs with correct labels', () => {
    render(<SubTabBar tabs={baseTabs} activeTab="meals" onTabChange={() => {}} />);
    expect(screen.getByText('Meals')).toBeInTheDocument();
    expect(screen.getByText('Nutrition')).toBeInTheDocument();
  });

  it('sets aria-selected="true" on the active tab', () => {
    render(<SubTabBar tabs={baseTabs} activeTab="nutrition" onTabChange={() => {}} />);
    expect(screen.getByTestId('subtab-nutrition')).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByTestId('subtab-meals')).toHaveAttribute('aria-selected', 'false');
  });

  it('calls onTabChange with correct id when a tab is clicked', () => {
    const onTabChange = vi.fn();
    render(<SubTabBar tabs={baseTabs} activeTab="meals" onTabChange={onTabChange} />);
  ...
  it('renders icons when provided', () => {
    render(<SubTabBar tabs={tabsWithIcons} activeTab="meals" onTabChange={() => {}} />);
  ...
  it('applies custom className to the container', () => {
    render(<SubTabBar tabs={baseTabs} activeTab="meals" onTabChange={() => {}} className="my-custom-class" />);
  ...
  it('handles a single tab edge case', () => {
    ...
    render(<SubTabBar tabs={singleTab} activeTab="only" onTabChange={() => {}} />);
    expect(screen.getAllByRole('tab')).toHaveLength(1);
    expect(screen.getByText('Only Tab')).toBeInTheDocument();
    expect(screen.getByTestId('subtab-only')).toHaveAttribute('aria-selected', 'true');
  });
  ...
  it('handles many tabs (5+) without breaking', () => {
    ...
    render(<SubTabBar tabs={manyTabs} activeTab="tab-0" onTabChange={() => {}} />);
  ...
  it('sets aria-controls to tabpanel-{id} on each tab', () => {
    render(<SubTabBar tabs={baseTabs} activeTab="meals" onTabChange={() => {}} />);
    expect(screen.getByTestId('subtab-meals')).toHaveAttribute('aria-controls', 'tabpanel-meals');
    expect(screen.getByTestId('subtab-nutrition')).toHaveAttribute('aria-controls', 'tabpanel-nutrition');
  });
  ...
  it('mixed tabs — only tabs with icons render icon elements', () => {
    ...
    render(<SubTabBar tabs={mixedTabs} activeTab="with-icon" onTabChange={() => {}} />);
