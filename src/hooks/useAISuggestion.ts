...
export function useAISuggestion({
  ...
  const startSuggestion = useCallback(() => { void fetchSuggestion(true); }, [fetchSuggestion]);
  const regenerate = useCallback(() => { void fetchSuggestion(false); }, [fetchSuggestion]);

  const apply = useCallback((selectedMeals: { breakfast: boolean; lunch: boolean; dinner: boolean }) => {
